...
public class GridBehavior : MonoBehaviour
{
    ...
    void SetPath()
    {
        ...
        for (int i = step; step > -1; step--)
        {
            ...
            player.transform.position = tempObj.transform.position;
            path.Add(tempObj);
            x = tempObj.GetComponent<GridStat>().x;
            y = tempObj.GetComponent<GridStat>().y;
            tempList.Clear();
             //Vector3.MoveTowards(player.transform.position, tempObj.transform.position, speedPlayer * Time.deltaTime);
        }
    ...
    {
        float currentDisdance = scale * rows * columns;
        int indexNumber = 0;
        for (int i = 0; i < list.Count; i++) 
        {
            if (Vector3.Distance(targetlocation.position, list[i].transform.position) < currentDisdance)
            {
                currentDisdance = Vector3.Distance(targetlocation.position, list[i].transform.position);
                indexNumber = i;
            }
        }
